...
private static int[] solution(int[] A, int[] B, int N) {
	int a_size=A.length;
	int[] semiPrimesCount = new int[a_size];
	
	for (int i = 0; i < a_size; i++) {
		int element1=A[i];
		int element2=B[i];
		semiPrimesCount[i] = findSemiPrimInRange(element1, element2, N);
	}
	return semiPrimesCount;
}

private static int findSemiPrimInRange(int element1, int element2, int N) {
	int semi = 0;
...
private static boolean isSemiPrime(int num, int N) {
...
private static List<Integer> getSemiPrimes(int N) {
