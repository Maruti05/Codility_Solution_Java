 ...
 * river. You are given an array A consisting of N integers representing the
 * falling leaves. A[K] represents the position where one leaf falls at time K,
 * measured in seconds. The goal is to find the earliest time when the frog can
 * jump to the other side of the river. The frog can cross only when leaves
 * appear at every position across the river from 1 to X (that is, we want to
 * find the earliest moment when all the positions from 1 to X are covered by
 ...
 * fall in the river. For example, you are given integer X = 5 and array A such
 * that: A[0] = 1 A[1] = 3 A[2] = 1 A[3] = 4 A[4] = 2 A[5] = 3 A[6] = 5 A[7] = 4
 * In second 6, a leaf falls into position 5. This is the earliest time when
 * leaves appear in every position across the river. Write a function: class
 * Solution { public int solution(int X, int[] A); } that, given a non-empty
...
public class FrogRiverOne {
	public static void main(String[] args) {
		...
		System.out.println(solutionAdv(A, X));
	}

	private static int solutionAdv(int[] A, int X) {
